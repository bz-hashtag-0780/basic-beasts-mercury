		...
		if (!this.testnet) {
			...
			if (options.roleInfo) {
				...
				if (roles[Flow.Roles.PROPOSER] || roles[Flow.Roles.ALL]) {
					let address =
						roles[Flow.Roles.PROPOSER] || roles[Flow.Roles.ALL];
					let account = await this.getAccount(address);
					builders.push(
						fcl.proposer(await signer.authorize(account))
					);

					if (roles[Flow.Roles.ALL]) {
						builders.push(
							fcl.authorizations([
								await signer.authorize(account),
							])
						);
						builders.push(
							fcl.payer(await signer.authorize(account))
						);
					}
				}
				...
				if (
					...
					for (
						let a = 0;
						a < roles[Flow.Roles.AUTHORIZERS].length;
						a++
					) {

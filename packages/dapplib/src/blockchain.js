...
module.exports = class Blockchain {
	...
	static async get(env, tx, args) {
		...
		options.decode = true;
		let response = await flow.executeTransaction(
			DappScripts[tx](env.imports),
			options
		);
		return {
			callAccount: null,
	...
	static async post(env, tx, args) {
		let proposer =
			typeof env.roles.proposer === 'string'
				? env.roles.proposer
				: env.config.accounts[0];
		let roleInfo = {
			[Flow.Roles.PROPOSER]: proposer,
			[Flow.Roles.AUTHORIZERS]:
				env.roles.authorizers &&
				Array.isArray(env.roles.authorizers) &&
				env.roles.authorizers.length > 0
					? env.roles.authorizers
					: [proposer],
			[Flow.Roles.PAYER]:
				typeof env.roles.payer === 'string'
					? env.roles.payer
					: proposer,
		};
		let options = {
		...
		let flow = new Flow(env.config);
		let result = await flow.executeTransaction(
			DappTransactions[tx](env.imports),
			options
		);

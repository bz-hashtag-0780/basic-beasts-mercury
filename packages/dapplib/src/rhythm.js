...
Object.keys(flowConfig.contracts).forEach((key) => {
	chainContracts[key] =
		flowConfig.contracts[key].aliases[NETWORK_EMULATOR_KEY];
});
...
const testnetAccounts = ['0x4742010dbfe107da', '0x0', '0x1', '0x2'];
...
(async () => {
	...
	if (mode === MODE.DEFAULT || mode === MODE.TEST) {
		...
		Object.keys(flowConfig.contracts).forEach((key) => {
			if (flowConfig.contracts[key].aliases[NETWORK_TESTNET_KEY]) {
				chainContracts[key] =
					flowConfig.contracts[key].aliases[NETWORK_TESTNET_KEY];
			}
		});
		...
		if (mode != MODE.TEST) {
			emulatorInstance.stdout.on('data', (data) => {
				let d = data
					...
					.replace(/"/g, '')
					.replace(
						/time=[0-9]{4}-[0-9]{2}-[a-zA-Z0-9]{5}:[0-9]{2}:[0-9]{2}-[0-9]{2}:[0-9]{2}/g,
						''
					)
					.replace(/level=debug msg=/g, '')
					.replace(/level=info msg=/g, '')
					.replace(/level=warning msg=/g, '');
				console.log(d);
			});
		} else {
			emulatorInstance.stdout.on('data', (data) => {});
	...
	async function createTestAccounts() {
		...
		for (let a = 0; a < accountCount; a++) {
			...
			let account = await flow.createAccount(
				keyInfo,
				String(tokens) + '.0'
			);
	...
	function processContractFolders(folders) {
		return new Promise((resolve, reject) => {
			...
			try {
				dappConfig = JSON.parse(
					fs.readFileSync(dappConfigFile, 'utf8')
				);
			} catch (e) {
			...
			emitter.on('file', (filePath) => {
				for (let f = 0; f < folders.length; f++) {
					if (
						...
						let { code, contractNames, deps } =
							getContractDependencies(folders[f], filePath);
			...
			emitter.on('end', () => {
				...
				sorted.forEach((s) => {
					...
					if (
						s !== null &&
						!Object.keys(dappConfig.contracts).includes(s)
					) {
						queue.push({
							prefix: s.split('.')[0],
				...
				if (match.groups.import.includes('.cdc')) {
					...
					if (importSplit[importSplit.length - 2] == '".') {
						...
						importRefs.push(
							contractType +
								'.' +
								importSplit[importSplit.length - 1].replace(
									'.cdc"',
									''
								)
						);
						...
						importRefs.push(
							importSplit[importSplit.length - 2] +
								'.' +
								importSplit[importSplit.length - 1].replace(
									'.cdc"',
									''
	...
	async function generate(
		...
		return new Promise((resolve, reject) => {
			...
			outSource +=
				...
				NEWLINE;
			outSource +=
				'const fcl = require("@onflow/fcl");' + NEWLINE + NEWLINE;
			outSource +=
			...
			emitter.on('file', (filePath) => {
				if (filePath.endsWith('.cdc')) {
					let functionName = filePath.replace(
					...
					outSource +=
						...
						'`' +
						NEWLINE;
					outSource += Flow.replaceImportRefs(
						code,
						deployedContracts
					);
					outSource += TAB + TAB + '`;';
